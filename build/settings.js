...
function setupInteractivityApi() {
  ...
  const config = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_2__.getConfig)(namespace);
  ...
  const {
    ...
    actions: {
      ...
      checkboxChange: event => {
        const context = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_2__.getContext)();
        const isCheckbox = event?.target instanceof HTMLInputElement && 'checkbox' === event?.target?.type;
        if (!isCheckbox) {
      ...
      restCall: (restUrl, body, refreshAfter = false) => {
        if (state.isSaving) {
      ...
      eddLicenseAction() {
        const context = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_2__.getContext)();
        actions.restCall(config.restUrl + '/edd-license-action', {
